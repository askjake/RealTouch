...
class STB(object):
   ...
   def __init__(self, stb=None, ip=None, pin=None, args=None, name=None, prod=False):
      self.name = name
      self.stb = stb if stb else getattr(self, 'stb', None)
      self.rid  = None 
      self.ip = ip if ip else getattr(self, 'ip', None)
      self.port = None
      self.verbose = False
      self.prod   = prod
      self.login  = None
      self.passwd = None
      self.cid    = None
      self.pin = pin if pin else self.pin if hasattr(self, 'pin') else None
      ...
      if self.prod:
         ...
         if not ((bool(self.login) and bool(self.passwd))):
            ...
            if self.start_pairing() and "stbs" in base.keys() and self.name and self.name in base["stbs"].keys():
   ...
   def start_pairing(self):
    print(f'SHarting pairing with stb={self.stb} and ip={self.ip}')
    print(f'Name={self.name}, rid={self.rid}, port={self.port}, verbose={self.verbose}, prod={self.prod}')
    print(f'Login={self.login}, passwd={self.passwd}, cid={self.cid}')
    ...
    response = self.query_noauth(query)
    if not response or response.get("result") != 1:
        print("Error starting pairing, result", response and response.get("result"))
        return False

    return True

   def finish_pairing(self, pin):
        print(f'Finishing pairing with stb={self.stb} and ip={self.ip} and pin={self.pin}')
        print(f'Name={self.name}, rid={self.rid}, port={self.port}, verbose={self.verbose}, prod={self.prod}')
        print(f'Login={self.login}, passwd={self.passwd}, cid={self.cid}')
        

        query = {"command": "device_pairing_start", "receiver": self.rid, "stb": self.stb, "app": "My_App", "name": "My app", "type": "python", "id": "S9", "mac":self.mac}
        response = self.query_noauth (query)
        if response["result"] != 1:
          print ("Error start pairing, result", response["result"])
          return False
       
        query["command"] = "device_pairing_complete"
        query["pin"] = pin
        response = self.query_noauth (query)
        if response["result"] != 1:
